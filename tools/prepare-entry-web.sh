...
if [[ ! -d "$WORK/.git" ]]; then git clone https://github.com/entrylabs/entry-offline.git "$WORK"; fi
...
mkdir -p "$DEST/src/main/views" "$DEST/src/main/static" "$DEST/src/renderer" "$DEST/src/renderer_build" "$DEST/node_modules" "$DEST/licenses"
cp "$WORK/src/main/views/main.html" "$DEST/src/main/views/main.html"
cp -R "$WORK/src/main/static/." "$DEST/src/main/static/"
cp -R "$WORK/src/renderer/resources" "$DEST/src/renderer/resources"
cp -R "$WORK/src/renderer_build/." "$DEST/src/renderer_build/"

copy_module() {
  local module="$1" source="$WORK/node_modules/$1" target="$DEST/node_modules/$1"
  [[ -e "$source" ]] || { echo "Missing required module: $module" >&2; exit 2; }
